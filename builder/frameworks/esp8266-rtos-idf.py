...
mcu = board.get("build.mcu", "esp8266")
idf_variant = mcu.lower()

FRAMEWORK_DIR = platform.get_package_dir("framework-esp8266-rtos-idf")
TOOLCHAIN_DIR = platform.get_package_dir("toolchain-xtensa-lx106-elf")
...
def find_framework_service_files(search_path, sdk_config):
    ...
    for d in os.listdir(search_path):
        ...
        for f in os.listdir(path):
            ...
            if f == "linker.lf" and not os.path.basename(path).startswith("esp8266"):
    ...
    result["lf_files"].extend(
        [
            ...
            os.path.join(
                ...
                idf_variant,
                "ld",
                "%s_bss_fragments.lf" % idf_variant,
            ),
            os.path.join(
                FRAMEWORK_DIR,
                "components",
...
def build_bootloader():
    ...
    code_model = get_cmake_code_model(
        ...
        [
            ...
            "-DSDKCONFIG=" + fs.to_unix_path(os.path.join(env.subst("$PROJECT_DIR"), "sdkconfig")),
...
def create_version_file():
    ...
    if not os.path.isfile(version_file):
        with open(version_file, "w") as fp:
            fp.write(platform.get_package_version("framework-esp8266-rtos-idf"))
...
def parse_conf():
    sdkconfig=parse_define(os.path.join(env.subst("$PROJECT_DIR"), "sdkconfig"))
    env.Replace(
       SDKCONFIG=sdkconfig,
    )
...
def build_sc_idf():
    ...
    if "windows" not in get_systype():
        return
    additional_packages = [
        os.path.join(platform.get_package_dir("toolchain-xtensa-lx106-elf"), "bin"),
        platform.get_package_dir("tool-ninja"),
        os.path.join(platform.get_package_dir("tool-cmake"), "bin"),
    ]
    if "windows" in get_systype():
...
""".format(env.subst("$FRAMEWORK_DIR"),";".join([l for l in additional_packages]))
    project_dir = env.subst("$PROJECT_DIR")
    build_wit_idf_file = os.path.join(project_dir, "idf.bat")
    if not os.path.isfile(build_wit_idf_file):
...
env.Prepend(
    ...
    FLASH_EXTRA_IMAGES=[
        (
            board.get("upload.bootloader_offset", "0x0"),
